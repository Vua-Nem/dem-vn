...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $banners = Banner::where('type', Banner::BANNER_HOME)
            ->where('is_mobile', Banner::BANNER_IS_WEBSITE)
    ...
    public function categoryDetail($slug)
    {
        ...
        return view('font_end.category.detail')
    ...
    public function productDetail1()
    {
        ...
        $relatedProducts = Product::with(["images", "description"])->where("slug", '!=', "dem_foam_gap_3_goodnight_eva")->limit(3)->get();
        $countdown = CountDown::where('status', CountDown::ACTIVE)
        ...
        return view('font_end.product.detail')
    ...
    public function productDetail2()
    {
        ...
        $relatedProducts = Product::with(["images", "description"])->where("slug", '!=', "dem_foam_goodnight_galaxy")->limit(3)->get();
        $reviews = Review::where('status', Review::REVIEW_IS_ACTIVE)
            ->where('entity_id', $product->id)
            ->with('getUser', 'reviewImage', 'getProduct')->get();
        $productService = new ProductService();
        ...
        $seoContent = $productService->getProductSeoContent($product->id);
        $this->productMeta($product, $seoContent, $reviews);
        return view('font_end.product.detail3')
            ->with("attributeName", $attrGroup["attributeName"] ?? [])
            ->with("attributeGroup", $attrGroup["attributeGroup"] ?? [])
            ->with("relatedProducts", $relatedProducts)
            ->with("product", $product)
    ...
    public function productDetail3()
    {
        ...
        $relatedProducts = Product::with(["images", "description"])->where("slug", '!=', "dem_lo_xo_goodnight_4stars")->limit(3)->get();
        $reviews = Review::where('status', Review::REVIEW_IS_ACTIVE)
            ->where('entity_id', $product->id)
            ->with('getUser', 'reviewImage', 'getProduct')->get();
        $productService = new ProductService();
        ...
        $seoContent = $productService->getProductSeoContent($product->id);
        $this->productMeta($product, $seoContent, $reviews);
        return view('font_end.product.detail2')
            ->with("attributeName", $attrGroup["attributeName"] ?? [])
            ->with("attributeGroup", $attrGroup["attributeGroup"] ?? [])
            ->with("relatedProducts", $relatedProducts)
            ->with("product", $product)
    ...
    public function productDetail4()
    {
        ...
        $relatedProducts = Product::with(["images", "description"])->where("slug", '!=', "dem_foam_goodnight_massage_nhat_ban")->limit(3)->get();
        $reviews = Review::where('status', Review::REVIEW_IS_ACTIVE)
            ->where('entity_id', $product->id)
            ->with('getUser', 'reviewImage', 'getProduct')->get();
        $productService = new ProductService();
        ...
        $seoContent = $productService->getProductSeoContent($product->id);
        $this->productMeta($product, $seoContent, $reviews);
        return view('font_end.product.detail4')
            ->with("attributeName", $attrGroup["attributeName"] ?? [])
            ->with("attributeGroup", $attrGroup["attributeGroup"] ?? [])
            ->with("relatedProducts", $relatedProducts)
            ->with("product", $product)
    ...
    public function productDetail5()
    {
        ...
        $relatedProducts = Product::with(["images", "description"])->where("slug", '!=', "dem_foam_than_hoat_tinh_zinus_charcoal_foam")->limit(3)->get();
        $reviews = Review::where('status', Review::REVIEW_IS_ACTIVE)
            ->where('entity_id', $product->id)
            ->with('getUser', 'reviewImage', 'getProduct')->get();
        $productService = new ProductService();
        ...
        $seoContent = $productService->getProductSeoContent($product->id);
        $this->productMeta($product, $seoContent, $reviews);
        return view('font_end.product.detail5')
            ->with("attributeName", $attrGroup["attributeName"] ?? [])
            ->with("attributeGroup", $attrGroup["attributeGroup"] ?? [])
            ->with("relatedProducts", $relatedProducts)
            ->with("product", $product)
    ...
    public function productDetail($slug)
    {
        ...
        ksort($attrGroup["attributeGroup"]);
        $this->productMeta($product, $seoContent, $reviews);
        return view('font_end.product.newDetail')
            ->with("attributeName", $attrGroup["attributeName"] ?? [])
            ->with("attributeGroup", $attrGroup["attributeGroup"] ?? [])
            ->with("product", $product)
            ->with("reviews", $reviews);
    ...
    public function ary_sort(&$array)
    {
        foreach ($array as &$value) {
            if (is_array($value)) $this->ary_sort($value);
        }
        return ksort($array);
    }
    ...
    {
        return view("font_end.home.chinhsach");
    }
    ...
    public function search(Request $request)
    {
        ...
        $products = Product::where('name', 'like', '%'.$keyword.'%')
            ...
            ->where("products.status", Product::PRODUCT_STATUS_IS_ACTIVE)
            ->get();
        return view('font_end.home.search', compact('products', 'keyword'));
    }

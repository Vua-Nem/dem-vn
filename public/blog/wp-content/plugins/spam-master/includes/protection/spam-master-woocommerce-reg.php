	...
	global $wpdb;
		...
		if(!empty($current_user_id) && $current_user_id != '0'){
			...
			if(!empty($spam_new_user)){
				...
				$spam_username = 'none';
				$blog_threat_email = 'woo@'.rand(10000000, 99999999).'.wp';
			}
			...
			if (!empty($email)) {
				if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
					$blog_threat_email = wp_strip_all_tags(substr($email,0,256));
				}
				else{
					$blog_threat_email = 'woo@'.rand(100000000000,999999999999).'.wp';
				}
			}
			else{
				$blog_threat_email = 'woo@'.rand(100000000000,999999999999).'.wp';
		...
		if(!empty($is_buffer)){
			throw new Exception( __( '<strong>SPAM MASTER</strong> '.$spam_master_message.''));
			exit();
		}

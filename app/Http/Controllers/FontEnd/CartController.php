...
class CartController extends Controller
{
	...
	{
		return view('font_end.cart.show_cart');
	}
	...
	public function addCart(Request $request)
	{
		...
		Cart::add([
			...
			'qty' => $data["quantity"] ?? 1,
			'price' => $productVariant->compare_price,
			'weight' => 0,
			'options' => [
	...
	public function addVoucher(Request $request)
	{
		...
		if ($voucher != Voucher::VOUCHER_STATUS_IS_HIDDEN || $voucher != Voucher::VOUCHER_STATUS_IS_ACTIVE)
			return redirect()->back()->withErrors("Mã giảm giá không hoạt động!");
